...
class PasteRepository:
    ...
    def get_bookmark_ids(self, user_id: str) -> set[str]:
        with session_scope() as session:
            return set(
                session.scalars(
                    select(Bookmark.paste_id).where(Bookmark.user_id == user_id)
                ).all()
            )
    ...
    def get_owned_paste_ids(self, user_id: str) -> set[str]:
        with session_scope() as session:
            return set(
                session.scalars(
                    select(Paste.id).where(Paste.author_id == user_id)
                ).all()
            )
    ...
    def import_bookmarks(self, user_id: str, paste_ids: list[str]) -> int:
        ...
        with session_scope() as session:
            ...
            valid = (
                set(
                    session.scalars(
                        select(Paste.id).where(Paste.id.in_(paste_ids))
                    ).all()
                )
                if paste_ids
                else set()
    ...
    def add_bookmark(self, user_id: str, paste_id: str) -> bool:
        with session_scope() as session:
            if (
                session.get(Paste, paste_id) is None
                or session.get(Bookmark, (user_id, paste_id)) is not None
            ):
                return False
            session.add(Bookmark(user_id=user_id, paste_id=paste_id))
            return True
        ...
        with session_scope() as session:
            result = session.execute(
                delete(Bookmark).where(
                    Bookmark.user_id == user_id, Bookmark.paste_id == paste_id
                )
            )
    ...
    def get_user_pastes(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        with session_scope() as session:
            ...
            by_id = {
                p.id: record for p in rows if (record := self._record(p)) is not None
            }
    ...
    def get_bookmarked_pastes(self, user_id: str) -> list[dict[str, Any]]:
        with session_scope() as session:
            rows = session.scalars(
                select(Paste)
                .join(Bookmark, Bookmark.paste_id == Paste.id)
...
def create_repository(
    database_url: str, *, echo: bool = False, encryption_key: str | None = None
) -> PasteRepository:
