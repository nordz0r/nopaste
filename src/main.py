...
@app.get("/", summary="Главная страница", description="Отображает форму для создания нового nopaste.")
async def read_root(request: Request):
    return templates.TemplateResponse("index.html", {"request": request})

@app.post("/paste", summary="Создать новый nopaste", response_description="Перенаправление на страницу нового nopaste")
...
@app.get("/paste/{paste_id}", summary="Просмотреть nopaste", description="Отображает содержимое указанного nopaste.")
async def get_paste(request: Request, paste_id: str):
    content = storage.get(paste_id)
    if not content:
        raise HTTPException(status_code=404, detail="Paste not found")
    return templates.TemplateResponse("paste.html", {"request": request, "paste_id": paste_id, "content": content})

@app.get("/list", summary="Список всех nopaste", description="Отображает список всех созданных nopaste.")
async def list_pastes(request: Request):
    pastes = list(storage.keys())
    return templates.TemplateResponse("list.html", {"request": request, "pastes": pastes})
